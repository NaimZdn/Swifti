...
/// Based on the source code from Cristian Selog :
/// https://github.com/christianselig/Markdownosaur/blob/main/Sources/Markdownosaur/Markdownosaur.swift
/// Customed with the tutorial of Alfian :
/// https://www.youtube.com/watch?v=DYD6_3JD7jk
...
public struct MarkdownAttributedStringParser: MarkupVisitor {
    ...
    let highliter: Highlightr = {
        let highliter = Highlightr()!
        highliter.setTheme(to: "vs2015")
        return highliter
    }()
    ...
    mutating func parserResults(from document: Document) -> [ParserResult] {
        ...
        func appendCurrentAttrString() {
            if !currentAttrString.string.isEmpty {
                ...
                return results.append(.init(attributedString: currentAttrStringToAppend, isCodeBlock: false, codeBlockLanguage: nil))
            }
        }
    ...
    public func visitCodeBlock(_ codeBlock: CodeBlock) -> NSAttributedString {
        let result = NSMutableAttributedString(attributedString: highliter.highlight(codeBlock.code, as: codeBlock.language) ?? NSAttributedString(string: codeBlock.code))
